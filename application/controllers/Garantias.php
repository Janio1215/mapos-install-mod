...
class Garantias extends MY_Controller
{
    ...
    public function imprimirGarantiaVenda()
    {
        if (!$this->uri->segment(3) || !is_numeric($this->uri->segment(3))) {
            $this->session->set_flashdata('error', 'Item não pode ser encontrado, parâmetro não foi passado corretamente.');
            redirect('mapos');
        ...
        $this->data['custom_error'] = '';
        $this->load->model('mapos_model');
        $this->data['vendaGarantia'] = $this->garantias_model->getByIdVendaGarantia($this->uri->segment(3));
        $this->data['emitente'] = $this->mapos_model->getEmitente();

        $this->load->view('garantias/imprimirGarantiaVenda', $this->data);
    }
